...
import mne
from params import EPO_DIR, FIG_DIR, SPEC_NR_SECONDS
...
for i_sub, subject in enumerate(subjects):
    ...
    sfreq = raw_ssd.info["sfreq"]
    n_fft = int(SPEC_NR_SECONDS * sfreq)

    psd_obj = raw_ssd.compute_psd(
        fmin=1,
        fmax=45,
        n_fft=n_fft,
        method="welch",
    ...
    psd, freq = psd_obj.get_data(return_freqs=True)
