...
from params import SSD_DIR, CSV_DIR, SPEC_NR_SECONDS, \
    DATA_DIR, SPEC_PARAM_DIR, SSD_PARAM_DIR, EPO_DIR
...
def percentile_spectrum(raw, band=(8, 12), nr_lines=5, i_chan=0,
    ...
    psd, freq = psd_obj.get_data(return_freqs=True) #psd shape: (n_epochs, n_channels=1, n_freqs)
...
def get_participant_list(aspect, condition):
    ...
    if aspect == 'data':
        ...
        for i_sub, subject in enumerate(subjects):
            fif_fname = EPO_DIR / f"{subject}_{condition}-raw.fif"
