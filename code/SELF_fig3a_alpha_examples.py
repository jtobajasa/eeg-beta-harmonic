...
import mne
from params import EPO_DIR, FIG_WIDTH, FIG_DIR, SPEC_NR_SECONDS, \
    SSD_PARAM_DIR
import numpy as np
import ssd
import pandas as pd
from SELF_helper import percentile_spectrum, despine
...
for i_sub, (subject, condition) in enumerate(subject_list):
    ...
    s = pd.read_csv(f'{SSD_PARAM_DIR}/{subject}_{condition}.csv', index_col=0).iloc[:,0]
    # df = df.set_index('subject')

    peak = float(s['alpha_peak'])
    file_name = f"{EPO_DIR}/{subject}_{condition}-raw.fif"
